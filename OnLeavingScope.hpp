...
public:
    ...
    OnLeavingScope(const Func& f) : m_func(f), m_owner(true) {}
    OnLeavingScope(Func&& f) : m_func(std::move(f)), m_owner(true) {}
    ~OnLeavingScope()
    {
        if (m_owner) {
            m_func();
 ...
 */
template <typename Func> detail::OnLeavingScope<typename std::decay<Func>::type> makeGuard(Func&& f)
{
    return detail::OnLeavingScope<typename std::decay<Func>::type>(std::forward<Func>(f));
}
