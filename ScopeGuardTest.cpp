...
double returnsDouble() noexcept { return 0.0; }
...
TEST(ScopeGuard, DifferentWaysToBind)
{
    ...
    EXPECT_EQ(0U, v.size()); // push_back happened on a copy of v... fail!
...
{
    EXPECT_DEATH((void)makeGuard([] { throw std::runtime_error("dtors should never throw!"); }),
...
void testFinally(ErrorBehavior error)
{
    ...
    try {
        ...
        (void)guard;
#else
        SCOPE_EXIT { cleanupOccurred = true; };
#endif
...
public:
    ...
    {
        try {
            // XXX auto e = std::current_exception();
            int test = 0;
            {
                SCOPE_EXIT { ++test; };
                EXPECT_EQ(0, test);
